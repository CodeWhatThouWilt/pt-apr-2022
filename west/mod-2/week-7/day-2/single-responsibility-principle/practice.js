...
function bakePies(pieQuantity, pieType, recipe) {
	for (let i = 0; i < pieQuantity; i++) {
		...
		combiningMsg += recipe.map((ingredient) => ingredient.name).join(", "); // Combining ingredients for .. pie crust, sugar, butter
...
function handleRevenues(totalCost, profitMargin, pieQuantity) {
...
function bakeAndSellPies(pieType, pieQuantity, profitMargin) {
	...
	const costOfPie = getCosts(recipe);
	...
	handleRevenues(totalCost, profitMargin, pieQuantity);
}
