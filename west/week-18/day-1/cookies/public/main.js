...
function findCookieVal(key) {
	if (document.cookie) {
		const cookies = document.cookie.split("; "); // ["name=yake", "theme=dragon"]
		const themeCookie = cookies.find((cookie) => cookie.startsWith(key)); // "theme=dragon"
		if (themeCookie) {
			return themeCookie.split("=")[1]; // dragon
		}
	}
}
...
function restoreName() {
	...
	val && setInputValue("display-name", val);
}
