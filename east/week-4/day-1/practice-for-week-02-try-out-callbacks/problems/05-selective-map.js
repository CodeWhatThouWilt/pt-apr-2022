...
// if statement to check if the result of first callback(selector is true)
// if above is true call the second callback(mapper) on the current element
// if above is not true push element into new array
...
function selectiveMap(array, selector, mapper) {
    ...
    for (let i = 0; i < array.length; i++) {
        ...
        const isTrue = selector(element);

        if (isTrue) {
            const res = mapper(element);
            newArr.push(res);
        } else {
            newArr.push(element);
        }
